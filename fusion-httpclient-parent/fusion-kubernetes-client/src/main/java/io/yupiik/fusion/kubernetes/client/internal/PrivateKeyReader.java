...
public final class PrivateKeyReader {
    ...
    public static PrivateKey readPrivateKey(final String pem) throws IOException {
        return readPEMObjects(pem).stream()
                .map(object -> switch (PEMType.fromBegin(object.beginMarker())) {
                    case PRIVATE_KEY_PKCS1 -> rsaPrivateKeyFromPKCS1(object.derBytes());
                    case PRIVATE_EC_KEY_PKCS8 -> ecPrivateKeyFromPKCS8(object.derBytes());
                    case PRIVATE_KEY_PKCS8 -> {
                        try {
                            yield rsaPrivateKeyFromPKCS8(object.derBytes());
                        } catch (final RuntimeException re) {
                            try {
                                yield ecPrivateKeyFromPKCS8(object.derBytes());
                            } catch (final RuntimeException re2) {
                                re.addSuppressed(re2);
                                throw re;
                            }
                        }
                    }
