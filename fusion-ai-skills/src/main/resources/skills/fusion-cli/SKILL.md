...
- Bundle `fusion-build-api` + `fusion-processor` (provided) and `fusion-api` + `fusion-cli` (compile) into
  the module.
- Shade into a one-jar with `Main-Class` set. Keep it runtime-dependency-light.<!-- generated from fusion-build-internal src/main/resources/agents/skills/fusion-cli/SKILL.md - do not edit -->
