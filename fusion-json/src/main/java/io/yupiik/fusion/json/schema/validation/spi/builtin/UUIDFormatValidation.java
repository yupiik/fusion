...
public class UUIDFormatValidation implements ValidationExtension {

    @Override
    public Optional<Function<Object, Stream<ValidationResult.ValidationError>>> create(final ValidationContext model) {
        if ("string".equals(model.schema().get("type")) && "uuid".equals(model.schema().get("format"))) {
            return Optional.of(new UUIDFormatValidation.Impl(model.toPointer(), model.valueProvider()));
        }
    ...
    static class Impl extends BaseValidation {
        ...
        protected Stream<ValidationResult.ValidationError> onString(final String value) {
            try {
                ...
                return Stream.of(new ValidationResult.ValidationError(pointer, value + " is not a UUID format"));
            }
        }
