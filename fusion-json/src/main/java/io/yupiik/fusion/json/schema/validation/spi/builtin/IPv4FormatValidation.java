...
public class IPv4FormatValidation implements ValidationExtension {

    @Override
    public Optional<Function<Object, Stream<ValidationResult.ValidationError>>> create(final ValidationContext model) {
        if ("string".equals(model.schema().get("type")) && "ipv4".equals(model.schema().get("format"))) {
            return Optional.of(new IPv4FormatValidation.Impl(model.toPointer(), model.valueProvider()));
        }
    ...
    static class Impl extends BaseValidation {
        ...
        protected Stream<ValidationResult.ValidationError> onString(final String value) {
            try {
                ...
                return Stream.of(new ValidationResult.ValidationError(pointer, value + " is not a IPv4 format"));
            }
        }
