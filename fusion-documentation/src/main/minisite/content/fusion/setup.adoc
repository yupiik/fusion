...
[abstract]
This page details Setup your project.
...
If it happens (`java: java.lang.IllegalArgumentException: Unsupported type: 'com.superbiz.MyJsonModel', known models: [....]` at compile time or `NoClassDefFoundError`/`No bean matching type '...'` at test/runtime for example), then just `Rebuild` the project, command is in `Build` menu (shortcut: `Alt+B` -> `R` by default).
...
The fusion annotation processor (`FusionProcessor`) accepts a set of compile-time options you can pass through `-A<key>` (maven-compiler-plugin `compilerArgs` or `annotationProcessorOptions`). They let you tune what is generated:

[opts="header",cols="a,a"]
|===
| Option | Purpose
| `fusion.skipNotes` | Disable the generated `@Note` on generated code.
| `fusion.workdir` | Enforces the work directory of the generator.
| `fusion.generateModule` | Generate the `FusionModule` implementation (`true` by default when possible).
| `fusion.moduleAppend` | Deprecated compatibility option, ignored.
| `fusion.moduleFqn` | Fully qualified name of the generated module.
| `fusion.generateBeanForCliCommands` | Generate beans for `@Command` beans.
| `fusion.generateBeanForHttpEndpoints` | Generate beans for `@HttpMatcher` endpoints.
| `fusion.generateBeanForJsonRpcEndpoints` | Generate beans for `@JsonRpc` methods.
| `fusion.generateBeanForPersistenceEntities` | Generate persistence entity models.
| `fusion.generatePartialOpenRPC` | Generate the partial OpenRPC documents.
| `fusion.generateJsonSchemas` | Generate the JSON schemas (`META-INF/fusion/json/schemas.json`).
| `fusion.generateBeanForJsonCodec` | Generate the JSON codecs for `@JsonModel` records.
| `fusion.generateConfigurationDocMetadata` | Generate the configuration documentation metadata.
| `fusion.generateBeanForRootConfiguration` | Generate the configuration beans for `@RootConfiguration` records.
| `fusion.generateNativeImage` | Generate GraalVM native-image reachability metadata.
| `fusion.skipLoadingModules` | Skip loading external fusion modules during generation.
| `fusion.debug.timeTracking` | Log the generation time of each phase.
|===
