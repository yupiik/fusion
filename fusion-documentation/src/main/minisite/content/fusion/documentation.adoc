...
[abstract]
This page details Documentation generation.
...
== Integration with yupiik-tools-maven-plugin

IMPORTANT: the main way to use these generators in practice is as **pre-actions of the `yupiik-tools-maven-plugin` `minisite` goal**, not as standalone programs. The plugin runs them before rendering the site, letting you embed generated Asciidoc (configuration keys, CLI command reference) directly into your minisite.

Here is the wiring used by this very project (`fusion-documentation/pom.xml`): each generator is declared as a `<preAction>`, and the generated partials land in `src/main/minisite/content/_partials/generated/` so they can be `include::`d from the hand-written pages:
...
<plugin>
  ...
  <executions>
    <execution>
      <id>minisite</id>
      <goals><goal>minisite</goal></goals>
      <configuration>
        <source>src/main/minisite</source>
        <preActions>
          <preAction>
...
Each generator consumes the metadata the Fusion annotation processor produced at compile time (`META-INF/fusion/configuration/documentation.json` for configuration, the CLI command metadata for `CliDocumentationGenerator`) and writes an `.adoc` partial which you then reference with an `include` directive from your content page (for example `{partialsdir}/generated/documentation.my-app.adoc`). Since the pre-action runs before the site renders, the partial exists by the time the page is resolved.

TIP: preview the site locally with `mvn compile yupiik-tools:serve-minisite -e` in the documentation module. The generated partials are committed (see the module rules): rebuild with `mvn install -pl fusion-documentation -am` so the processor regenerates the metadata before the site renders.
...
| formatter | Can be `list` for a plain asciidoc list (with h1/h2 titles), `table` for a table and `definitionlist` for a definition list. Data are globally the same but depending your configuration it can be easier to read in one style or another.
...
|===
| Name               | Description
| indexHeader        | Header - top content - of the index page.
| indexName          | Name if the index page listing commands, defaults to `index.adoc`. Can also include some relative path to render it in another folder but ensure to adjust `indexRelativePath` then.
| indexRelativePath  | Prefix to links to the command documentation, this is useful when the index page is generated in partials directory for example and you include it in another page.
