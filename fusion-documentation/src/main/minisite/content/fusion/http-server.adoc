...
The `fusion-http-server` module directly provides the classic health and metrics endpoints as `MonitoringEndpoint`
beans, in the API package `io.yupiik.fusion.http.server.observability`:
...
Then call `/health?type=<my type>` - for example `/health?type=live` - to run only those checks. Without the `type`
