...
@Retention(RUNTIME)
@MonoFusionSupport
@ExtendWith(MyAppSupport.MyConf.class)
...
`@Task(value)` must implement `Task.Supplier`, which can be a java `Supplier<T>` - which, for `BEFORE` tasks, will enable to inject some state as parameter referencing the bean class.
It can also be a `Runnable` - mainly for `AFTER` tasks.
...
----
@DefaultScoped
public class BeforeTask implements Task.Supplier<MyData> {
...
`fusion-testing` ships `TestingModule` and `ListingPredicateModule` which let you replace application beans with test doubles. `TestingModule` scans the generated `$FusionBean` classes of the current project (from `target/classes` and `target/test-classes`) and swaps the matched bean for your replacement, so you can override any `@ApplicationScoped`/`@DefaultScoped` bean without reflection.

[source,java]
----
final var container = ConfiguringContainer.of()
    .disableModule(/**/ TestingModule.class) <1>
    .start();
----
<.> Register your own `TestingModule` instance (or a `ListingPredicateModule`) to replace the matching beans before the container starts.
