...
A `DatabaseTranslation` SPI adapts the generated SQL to the target database. Fusion ships:

* `DefaultTranslation` - the default, generic dialect,
* `H2Translation`, `MySQLTranslation`, `PostgresTranslation`, `OracleTranslation`, `DuckDBTranslation`.

When you create a database through `DatabaseFactory`/`DatabaseConfiguration`, the dialect is auto-detected from the JDBC URL; if none is recognized you get an explicit error suggesting `DefaultTranslation`.
...
Fusion recommends to use a UUID or equivalent as identifier, hence the `String` type here, the `onInsert` hook below generates it when missing.
When mapping an existing database with an auto-incremented key you can instead use a numeric type with `@Id(autoIncremented = true)`: it relies on `java.sql.Statement.getGeneratedKeys` to map the key.
...
----
    ...
    @OnUpdate
    public CustomerEntity onUpdate() {
        return this;
    }
...
[source,java]
----
@Table("ignored") // project-only entity, the table name is ignored for the query
